...
- [2. smart pointer](#2-smart-pointer)
  - [shared_ptr & weak_ptr](#shared_ptr--weak_ptr)
- [3. new enum](#3-new-enum)
- [4. cast type](#4-cast-type)
  - [static_cast、dynamic_cast、reintepret_cast、const_cast  std::remove_const<>](#static_castdynamic_castreintepret_castconst_cast-stdremove_const)
- [5. lvalue & rvalue](#5-lvalue--rvalue)
- [6. move & forward](#6-move--forward)
- [7. variadic template](#7-variadic-template)
- [8. tuple](#8-tuple)
- [9. functional & bind](#9-functional--bind)
...
- [14. thread_local](#14-thread_local)
- [15. singleton](#15-singleton)
    ...
    - [c++11 instroduces series of ways to solve this issue:](#c11-instroduces-series-of-ways-to-solve-this-issue)
      - [calling "std::call_once"](#calling-stdcall_once)
...
- [16. Alignof & alignment_of](#16-alignof--alignment_of)
...
- [ctkernel->_tresult: send rsplogin #msg:Tid_RspAccountDeposit](#ctkernel-_tresult-send-rsplogin-msgtid_rspaccountdeposit)
...
+ 在Linux系统里，/usr/include/linux/if_pppox.h里面有这样一个结构：
    ...
    + allocate:
    ![avatar](engineer_png/allocator_alloc.png)

    + fixmem init:
    ![avatar](engineer_png/fixmem_init.png)

    + fixmem alloc:
     ![avatar](engineer_png/fixmem_alloc.png)

    + memorydb alloc:
     ![avatar](engineer_png/mdb_alloc.png)

    + memorydb remove:
     ![avatar](engineer_png/mdb_remove.png)
...
### c++11 instroduces series of ways to solve this issue:
